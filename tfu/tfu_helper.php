...
@ob_start();
// neede to be here because of possible depreciate warning!
...
function tfu_debug($data)
{
    ...
    global $enable_enhanced_debug;
      ...
      if(stristr($data, 'called statically') === false && stristr($data, 'is deprecated') === false) { // This error can happen in Joomla and can be ignored
...
function getDestinationFolder($id_list)
{
    ...
    foreach ($ids as $id) {
        ...
        while (false !== ($filed = readdir($dirhandle))) {
            if ($filed != '.' && $filed != '..' && !in_array($filed, $exclude_directories) && (!($hide_hidden_files && (strpos($file, '.') === 0)))) {
                if (is_dir($dir . '/' . $filed)) {
